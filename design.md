...
3. **Attack Lab**
   ...
   Loads reproducible attack results generated by a command-line experiment runner and focuses its charts and tables on B1 versus B3. Presentation mode uses deterministic saved scenarios so the result does not depend on testnet availability. The web application does not directly start long-running experiments.
...
Experiments run through a separate command-line program and write to an isolated result directory or temporary database. The Presentation API only reads frozen experiment results and never shares mutable grant, payment, or replay-prevention state with an experiment run.
...
The scorer produces an assessment such as:
...
The assessment also carries a basis:
...
All submitted, non-revoked feedback in the supported `quality` dimension is aggregated without requiring proof of interaction.

### B2: Payment-Grounded Reputation

Only feedback with a valid, unique, service-bound payment receipt contributes to the score.

### B3: Full RepuGate

B3 extends B2 with a confidence-aware and Sybil-aware weighting policy. Candidate factors include:

- number of verified interactions
- reviewer diversity
- per-reviewer influence cap
- time decay
- burst behaviour penalty
- common funding-source penalty
- service identity age
...
Payment amount is not weighted linearly: a provider can recover most funds from self-payments and would otherwise be able to buy reputation cheaply. Distinct verified payers, capped per-reviewer influence, transaction timing, and funding concentration are treated as risk indicators rather than proofs of independent users.
...
B1 and B3 must be compared at the same or similar honest-service approval rate. Otherwise, a system could appear safe merely by blocking most requests.

The main comparison is:

```text
At the same honest-service approval rate,
does B3 produce a lower malicious payment rate than B1?
```
...
├── experiments/                  # B1/B3, attack scenarios, and CLI runner
├── contracts/
...
- B1 versus B3 as the core presentation comparison, with ungrounded-feedback, receipt-replay, and offer-substitution attacks
- payment-grounded and Sybil-aware claims
