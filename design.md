...
3. **Attack Lab**
   ...
   Loads reproducible attack results generated by a command-line experiment runner and compares B0, B1, B2, B3-Beta, and B3-Dirichlet. Presentation mode uses deterministic saved scenarios so the result does not depend on testnet availability. The web application does not directly start long-running experiments.
...
They must be separated from RepuGate's shared payment controls. B0 through B3
...
B0/B1/B2/B3 demonstrates the common gateway, not a superior reputation model.
...
keeps the B1/B2/B3 reputation comparison controlled: those models estimate the
same property, while the independent variable is how feedback is validated and
aggregated. B0 is the no-reputation reference. Identity binding, payment
validity, reviewer diversity, and confidence are evidence or risk signals and
therefore remain separate from the quality score.
...
### B0: No Reputation Gate (Implemented Reference Baseline)

**Purpose.** B0 measures what happens when the Buyer follows the same protected
x402 payment path but makes no reputation-based trust decision.

**Input and calculation.** B0 resolves the service identity only for the shared
offer/payee/endpoint binding, but it does not load feedback, calculate a score
or confidence, or invoke payment-evidence verification. Its score and
confidence fields are explicitly `null`, not zero.

**Decision.** After the common offer, identity, budget, network, and redirect
checks pass, B0 returns `ALLOW` with reason `REPUTATION_GATE_DISABLED`. The API
then issues the same exact-offer-bound, short-lived, one-use EvaluationGrant
used by the other baselines. A Provider with no reputation or actively
manipulated reputation is therefore treated like an honest Provider.

**Attack coverage.** B0 inherits the common gateway's malformed-offer, budget,
exact-binding, Grant-replay, and payment-state controls. It does not address
ungrounded feedback, receipt replay as reputation evidence, reviewer
concentration, paid Sybil self-review, collusion, identity whitewashing, or
service trustworthiness.

**Implementation status.** B0 is implemented in the shared Core evaluator, API,
deterministic Buyer flow, experiment runner, and Attack Lab.
...
| Offer/recipient/endpoint substitution | shared gate | shared gate | shared gate | shared gate | shared gate | Canonical offer and identity checks, not reputation scoring |
| EvaluationGrant replay or altered authorization | shared gate | shared gate | shared gate | shared gate | shared gate | One-use Grant and field-by-field intent recheck |

### Deterministic Fixture Outcomes

This table maps the implemented attack fixtures to every baseline and the
frozen thresholds. A dash means that B0 intentionally produces no reputation
score.

| Fixture | B0 | B1 | B2 | B3-Beta | B3-Dirichlet | Main reason |
| --- | --- | --- | --- | --- | --- | --- |
| Honest | ALLOW (—) | ALLOW | ALLOW | ALLOW (74%) | ALLOW (76%) | B0 ignores reputation; the other evidence is honest |
| Ungrounded ratings | ALLOW (—) | ALLOW | BLOCK | BLOCK | BLOCK | B0/B1 do not require payment proof; B2/B3 reject ungrounded ratings |
| Receipt replay | ALLOW (—) | ALLOW | BLOCK | BLOCK (36.7%) | BLOCK (26.7%) | B2/B3 admit only one rating for the reused receipt |
| Reviewer concentration | ALLOW (—) | ALLOW | ALLOW | BLOCK (40%) | BLOCK (36%) | B3 removes same-wallet multiplicity and then fails its score gate |
| Expected-offer mismatch | BLOCK (—) | BLOCK | BLOCK | BLOCK | BLOCK | The common canonical-offer gate blocks before payment, independently of reputation score |
...
B0 uses no reputation thresholds. B1 and B2 use the score thresholds; their
confidence is diagnostic only. Both B3 variants require both score and
confidence for `ALLOW`. A score of at least 70% with
insufficient B3 confidence returns `REVIEW`; a score below 50% returns `BLOCK`.
Any identity or exact-offer binding failure is a hard `BLOCK` for all five
...
each model. The current `75% / 75% / 25% / 0% / 0%` adversarial allow rates mean
only that B0/B1/B2/B3-Beta/B3-Dirichlet allow `3 / 3 / 1 / 0 / 0` of the four
fixed fixtures; they are not population estimates. B0 and B1 tie on this small
set because all reputation attacks inflate raw scores; B1's additional behavior
must be measured with low-reputation and no-reputation controls in the expanded
experiment.
...
All five baselines must be compared at the same or similar honest-service
approval rate. Otherwise, a system could appear safe merely by blocking most
requests.
...
1. what changes when B1 uses raw reputation relative to B0's no-gate behavior?
2. does B2 reduce ungrounded and replay-based payments relative to B1?
3. do both B3 variants reduce reviewer-concentration payments relative to B2?
4. how do the Beta expected-quality and Dirichlet Good-or-better semantics
...
and raw JSON/CSV are saved; and all five models allow the single honest control
...
adversarial fixtures, evaluated once under B0, B1, B2, B3-Beta, and
B3-Dirichlet for 25 result rows.
...
- five-model comparison cards and attack matrix generated from frozen experiment data
...
├── experiments/                  # B0/B1/B2/B3, attack scenarios, and CLI runner
├── contracts/
...
- B0/B1/B2/B3-Beta/B3-Dirichlet Presentation baselines
...
- B0/B1/B2/B3-Beta/B3-Dirichlet as the core presentation comparison, with ungrounded-feedback, receipt-replay, reviewer-concentration, and offer-substitution attacks
